...
function AddSocialFrom(props) {
  ...
  async function DeleteSocial() {
    ...
    let url = "http://localhost:8080/business/deleteSocial/" + params.socialId;
    let method = "PUT";
  ...
  async function registerSocial(event) {
    ...
    if (params.socialId) {
      url = "http://localhost:8080/business/editSocial/" + params.socialId;
      method = "PUT";
    } else {
      url =
        "http://localhost:8080/business/addSocial/" + props.location.state.id;
      method = "POST";
    }
  ...
  async function FetchSocial() {
    ...
    try {
      // prettier-ignore
      const response = await fetch("http://localhost:8080/business/getSocial/" + params.socialId);
      // prettier-ignore
